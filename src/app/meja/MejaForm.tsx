...
export default function MejaForm({ initialData, ruangLabs, assignedLabIds, userRole }: MejaFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = initialData ? `/api/meja/${initialData.id}` : "/api/meja";
      const method = initialData ? "PUT" : "POST";

      const res = await fetch(url, {
        method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ meja, ruangLabId }),
      });
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">
          {initialData ? "Edit Meja" : "Tambah Meja"}
        </h2>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              value={ruangLabId}
              onChange={(e) => setRuangLabId(e.target.value)}
              required
